...
const SaleScreen = ({ navigation }) => {
  ...
  const {
    state: { menus, currentOrder, order, isLoading },
    fetchMenuSale,
    saveFavorite,
    fetchCurrentOrder,
    fetchOrderById,
  ...
  return (
    <View style={styles.container}>
      ...
      <SearchBar
        ...
        lightTheme
        onChangeText={(search) => {
          fetchMenuSale(category.id, search ? search : 'ALL', category.name);
      ...
      >
        <View style={styles.sectionCategories}>
          <Text style={styles.textCap}>Categories</Text>
          <SelectDropdown
            ...
            onSelect={async (selectedItem, index) => {
              ...
              await fetchMenuSale(
                selectedItem.id,
                menu,
                selectedItem.name !== 'Favorite' ? selectedItem.id : 'Favorite'
              );
            }}
        ...
        <FlatGrid
          ...
          data={menus}
          renderItem={({ item }) => (
            <View key={item.id} style={styles.prodCardContent}>
              <TouchableOpacity
                onPress={() => {
              ...
              <View style={styles.footerCard}>
                ...
                <Text style={styles.textPrice}>
                  {currentOrder.orderType === 'partner'
                    ? currency(item.specialPrice, { separator: ',' }).format()
                    : currency(item.price, { separator: ',' }).format()}
                </Text>
              </View>
            </View>
          )}
        ...
        onSubmit={({ quantity }) => {
          ...
          if (type === 'protein') {
            ...
            createOrderDetail({
              ...
              Toast.show({
                text: 'This menu is already to cart.',
                buttonText: 'Okay',
                type: 'success',
              });
              fetchOrderById(currentOrder.id);
