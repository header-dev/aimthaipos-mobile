...
import { Container, Body, Toast } from 'native-base';
...
const PrinterBillEditScreen = ({ navigation }) => {
  ...
  const retrieveData = async () => {
    try {
      const valueString = await AsyncStorage.getItem('kitchen_printer');
      if (valueString) {
  ...
  const saveData = async (value) => {
    try {
      await AsyncStorage.setItem('kitchen_printer', JSON.stringify(value));
      Toast.show({
  ...
  return (
    <Container style={{ backgroundColor: '#fff' }}>
      <Header
        ...
        centerComponent={{
          text: 'Kitchen Printer Preferences',
...
export default withNavigation(PrinterBillEditScreen);
