...
import { BOLD_OFF, BOLD_ON, TXT_ALIGN_CT, TXT_DOUBLE, TXT_DOUBLE_OFF, TXT_2HEIGHT, TXT_2HEIGHT_OFF } from "../../constants";
import { Alert } from "react-native";
import { Divider } from "react-native-paper";

const CheckoutScreen = ({ navigation }) => {

  const { state: { order, isLoadingOrderDetail, currentOrder }, fetchOrderById, clearCurrentOrder, updateOrder } = useContext(SaleContext);
  const { state: { card, isCardLoading, isCardRejected }, fetchCard } = useContext(SettingContext)
  const { state: { shop }, fetchShop } = useContext(ShopContext)
  ...
  const menuItem = (item, price) => item + price.padStart(32 - item.length, ".")
  ...
  const printBill = async () => {
    try {
      ...
      const total = totalAmount + serviceCharge - order.promotionAmount + cardCharge + order.tip
      ...
      ).format()}\nRounding: ${order.diffRoundup ? order.diffRoundup : 0.00}\nTotal Price:${currency(totalPrice, {
        separator: ",",
      }).format()}\n================================\n${paymentDetail}================================</C>\n`
    ...
    if (values.returnAmount >= 0) {
      const totalAmount = _.sumBy(order.orderDetails, "totalAmount") +
        ((order.serviceCharge / 100) *
          _.sumBy(order.orderDetails, "totalAmount")) - order.promotionAmount + order.tip
  ...
  const paymentStamp = (type) => {
    ...
    let orginalTotal = _.sumBy(order.orderDetails, "totalAmount") +
      ((order.serviceCharge / 100) *
        _.sumBy(order.orderDetails, "totalAmount")) - order.promotionAmount + order.tip
    let diffRoundup = 0.00
    ...
    updateOrder(_order.id, {
      ...
      pay: orginalTotal,
      cardCharge: type === "card" ? card.fee : 0.00,
      payChange: type !== "cash" && 0.00,
      payReceive: type !== "cash" && 0.00,
    }).then(() => {
    ...
    <SafeAreaView style={styles.container} forceInset={{ top: "always" }}>
      ...
      <Header
        ...
        rightComponent={() => (
          <View style={{ flexDirection: 'row' }}>
            <TouchableOpacity style={{}} onPress={() => { navigate("Calculator") }}>
      ...
      <View style={{ flex: 1, flexDirection: "row" }}>
        <View style={styles.contentItem}>
          {isLoadingOrderDetail && <ActivityIndicator size="small" style={{ margin: 5 }} />}
          <FlatList
            ...
            ListFooterComponent={() => (
              <View style={styles.footerList}>
                <View style={styles.textTotal}>
                  ...
                  <Text>
                    {currency(parseFloat(_.sumBy(order.orderDetails, "amount")), {
                      separator: ",",
                    }).format()}
                  </Text>
                </View>
                <View style={styles.textTotal}>
                  ...
                  <Text>
                    ...
                    {currency(order.promotionAmount, {
                      separator: ",",
                    }).format()} ({order && order.promotion ? order.promotion.name : 0})
                  </Text>
                </View>
                <View style={styles.textTotal}>
                  <Text style={styles.textLabel}>Adjustment</Text>
                  <Text>{currency(order.tip, {
                    separator: ",",
                  }).format()}</Text>
                </View>
                <View style={styles.textTotal}>
          ...
          <View style={{
            ...
            <PaymentForm
              initialValue={{
                payReceive: 0.0,
                returnAmount: 0.0,
                amountDue: _.sumBy(order.orderDetails, "totalAmount") +
                  (order.serviceCharge / 100) *
                  _.sumBy(order.orderDetails, "totalAmount") +
                  ((order.cardCharge / 100) *
                    _.sumBy(order.orderDetails, "totalAmount"))
          ...
          <View style={{
              ...
              {order.orderType !== "partner" ? (
                <>
                  <Button
                    ...
                    buttonStyle={[styles.buttonPayment, { backgroundColor: order.paymentType === "cash" ? "grey" : "transparent" }]}
                    onPress={() => {
                      paymentStamp("cash")
                    }}
                  />
                  <Button
                    ...
                    buttonStyle={[styles.buttonPayment, { backgroundColor: order.paymentType === "card" ? "grey" : "transparent" }]}
                    onPress={() => {
                      paymentStamp("card")
                    }}
                  />
                ...
                <Button
                  ...
                  buttonStyle={[styles.buttonPayment, { backgroundColor: order.paymentType === "partner" ? "grey" : "transparent" }]}
                  onPress={() => {
            ...
            <View style={{
              ...
              <Button
                ...
                onPress={() => {
                  if (printer) {
                    ...
                    Toast.show({
                      text:
                        "Please check to configuration to bill / receipt printer before.",
                      buttonText: "Okay",
                      type: "warning",
                    });
                  }
              ...
              <Button
                ...
                onPress={() => {
                  try {
                    if (order.paymentType === 'cash' && order.payChange <= 0 &&
                      order.payReceive <= 0) {
                      throw ""
                    }
                    ...
                    Alert.alert('Invalid Input', 'This Bill has applied the CASH payment, Please input the payment receive.', [])
                  }
                }}
              />
            </View>
