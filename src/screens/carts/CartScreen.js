...
import { BOLD_ON, BOLD_OFF, TXT_2HEIGHT, TXT_2HEIGHT_OFF, TXT_ALIGN_CT, TXT_2WIDTH, TXT_2WIDTH_OFF, TXT_DOUBLE, TXT_DOUBLE_OFF, TXT_ALIGN_LT, TXT_ALIGN_LT_OFF, TXT_ALIGN_RT, TXT_ALIGN_RT_OFF } from "../../constants";
...
const CartScreen = ({ navigation }) => {
      ...
      <ListItem bottomDivider key={item.id}
        ...
        <ListItem.Content>
          <View style={{ flexDirection: 'row' }}>
            <ListItem.Title h4>
              {item.product.name} {item.makeTakeAway || order.orderType === "take-away" ? "(TA)" : ""} x {item.quantity}
            </ListItem.Title>
          </View>
          <ListItem.Subtitle style={{ fontStyle: 'italic' }}>
            Price : $ {priceNumberFormat(item.priceAdditionPrice)}
          </ListItem.Subtitle>
          <ListItem.Subtitle style={{ fontStyle: 'italic' }}>Discount : - {item.discount}</ListItem.Subtitle>
          {item.makePrinted && (
        ...
        {item.protein && (<ListItem.Content style={{ flexWrap: 'wrap', height: 100 }}>
          <ListItem.Title h4>Protien</ListItem.Title>
          <ListItem.Subtitle style={{ fontStyle: 'italic' }}>{item.protein.name}</ListItem.Subtitle>
        </ListItem.Content>)}
  ...
  const renderTotal = () => {
    ...
    let serCharge =
      (order.serviceCharge / 100) * totalAmount;
    let total = parseFloat(
      totalAmount + serCharge -
      order.promotionAmount
    );
    return currency(total, { separator: "," }).format();
  };

  const menuItem = (item, protein) => item + protein.padStart(25 - item.length, ".")
          ...
          },
          {
            text: "OK", onPress: async () => {
              ...
              updateOrder(order.id, {
                ...
                const orderPrint = _.orderBy(orderDetail, item => item.product.category.dataLevel, ['asc'])
                let allContent = ``
                let allContent2 = ``
                ...
                headerContent += `${TXT_ALIGN_CT}${TXT_2HEIGHT}${currentOrder && currentOrder.orderNo}      ${contentType}${TXT_2HEIGHT_OFF}${TXT_ALIGN_CT}\n`
                headerContent += `${TXT_ALIGN_CT}${TXT_2HEIGHT}${user && user.username}${TXT_2HEIGHT_OFF}${TXT_ALIGN_CT}\n`
                headerContent += `${TXT_ALIGN_CT}--------------------------------${TXT_ALIGN_CT}\n`
                let contentProduct = ``
                _.orderBy(orderPrint, "category.dataLevel").map(i => {
                  ...
                  if (i.orderRequest) {
                    ...
                    request = `${TXT_ALIGN_CT}${TXT_DOUBLE}${req.length ? '-' + req.filter(Boolean).join(",") : ''}${TXT_DOUBLE_OFF}${TXT_ALIGN_CT}${remark ? '\n' + TXT_ALIGN_CT + TXT_DOUBLE + remark + TXT_DOUBLE_OFF + TXT_ALIGN_CT : ''}\n`
                  }
                  let wordTakeAway = ``
                  if (i.makeTakeAway || order.orderType === "take-away" || order.orderType === 'partner') {
                    wordTakeAway = '(TA)'
                  }
                  contentProduct += `${TXT_ALIGN_LT}${TXT_DOUBLE}${i.quantity} - ${menuItem(wordTakeAway + i.product.name, i.protein ? i.protein.name : '')}${TXT_DOUBLE_OFF}${TXT_ALIGN_CT}${request ? '\n' + request : '\n'}`
                })
                ...
                allContent2 += `${TXT_ALIGN_CT}${TXT_2HEIGHT}${jobDate}${TXT_2HEIGHT_OFF}${TXT_ALIGN_CT}\n${TXT_ALIGN_CT}${TXT_2HEIGHT}${currentOrder && currentOrder.orderNo}      ${contentType}${TXT_2HEIGHT_OFF}${TXT_ALIGN_CT}\n`
                allContent2 += `${TXT_ALIGN_CT}${TXT_2HEIGHT}${user && user.username}${TXT_2HEIGHT_OFF}${TXT_ALIGN_CT}\n`
          ...
          <View style={styles.headerRow}>
            ...
            >
              ...
              <View style={styles.textTotal}>
                ...
                <Text>
                  - {order && currency(order.promotionAmount, {
                    separator: ','
                  }).format()} ({order && order.promotion && order.promotion.name})
                </Text>
              </View>
              <View style={styles.textTotal}>
          ...
          >
            {isLoadingOrderDetail ? (
              ...
              <FlatList
                ...
                data={order && _.orderBy(order.orderDetails, item => item.product.category.dataLevel, ['asc'])}
