...
const userReducer = (state, action) => {
    switch (action.type) {
        ...
        case 'get_dailysale_success':
            return {
                ...state,
                dailysales: action.payload, isLoading: false, isRejected: false
            }
        ...
        case 'get_sumtransitem_success':
            return {
                ...state,
                report_datas: action.payload, isLoading: false, isRejected: false
            }
        ...
        case 'get_sumitem_success':
            return {
                ...state,
                report_sum_item_datas: action.payload, isLoading: false, isRejected: false
            }
...
const fetchDailySaleReport = dispatch => (query = "") => {
    dispatch({ type: 'get_dailysale_fetch' })
    backendApi.get(`/report/daily-sales?q=${query}`).then(response => {
        dispatch({
            type: 'get_dailysale_success', payload: response.data
        })
    }).catch(err => {
...
const fetchSumSaleTransactionItem = dispatch => (query = "") => {
    dispatch({ type: 'get_sumtransitem_fetch' })
    backendApi.get(`/report/summary-sale-transaction-item?q=${query}`).then(response => {
        dispatch({
            type: 'get_sumtransitem_success', payload: response.data
        })
    }).catch(err => {
...
const fetchSumItem = dispatch => (date_f, date_e) => {
    ...
    backendApi.get(`/report/summary-sale-item?date_f=${date_f}&date_e=${date_e}`)
        .then(response => {
            dispatch({
                type: 'get_sumitem_success', payload: response.data
            })
        }).catch(err => {
