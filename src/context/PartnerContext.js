...
const userReducer = (state, action) => {
    switch (action.type) {
        ...
        case 'get_partner_success':
            return {
                ...state,
                partners: action.payload, isLoading: false, isRejected: false
            }
...
const fetchPartner = dispatch => (query = "") => {
    dispatch({ type: 'get_partner_fetch' })
    backendApi.get(`/partner?q=${query}`).then(response => {
...
const deletePartner = dispatch => (id) => {
    backendApi.delete(`/partner/delete/${id}`)
        .then((response) => {
